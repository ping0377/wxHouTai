  ...
  <div id="BoxOpen">
    ...
    <el-main style="height: calc(100vh - 68px); padding: 0px; padding-top: 0px;">
      ...
      <div style="width: 100%;">
        <el-button
          type="text"
          style="width: 32%; padding: 7px 0px; margin: 0px;"
          @click="getDoors()"
        >添 加</el-button>
        <el-button
          type="text"
    ...
    <el-dialog class="dialogBO" title="批量添加" :visible.sync="dialogFormVisible4" fullscreen>
      <dl>
        <dd class="item1">袋子名称:</dd>
        <dd class="item">
          <el-input placeholder="请输入袋子名称" v-model="addName" clearable></el-input>
        </dd>
      </dl>
      <dl>
        <dd class="item1">编号前缀:</dd>
        <dd class="item">
          <el-input placeholder="请输入编号前缀" v-model="addNumName" clearable></el-input>
        </dd>
      </dl>
      <dl>
        <dd class="item1">起始编号:</dd>
        <dd class="item">
          <el-input maxlength="14" placeholder="请输入起始编号" v-model.number="addNumStart" clearable></el-input>
        </dd>
      </dl>
      <dl>
        <dd class="item1">结束编号:</dd>
        <dd class="item">
          <el-input maxlength="14" placeholder="请输入结束编号" v-model.number="addNumEnd" clearable></el-input>
        </dd>
      </dl>
      <span slot="footer" class="dialog-footer">
        <el-button @click="dialogFormVisible4 = false">取 消</el-button>
        <el-button type="primary" @click="addDatas()">添 加</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  watch: {
    ...
    addIndex: {
      handler: function(val, oldVal) {
        if (!!val.split("boxIndex=")[1]) {
          this.addIndex = val.split("boxIndex=")[1];
        }
      },
      immediate: true
    }
  ...
  methods: {
    ...
    addDatas() {
      let Client = localStorage.getItem("ClientAddr");
      let boxInfo = []
      if (parseInt(this.addNumStart) < parseInt(this.addNumEnd)) {
        if(!!this.addName && !!this.addNumName){
          for (let i = parseInt(this.addNumStart);i <= parseInt(this.addNumEnd);i++) {
            boxInfo.push({nameBox:this.addName,boxIndex:this.addNumName + i});
          }
          let obj = {
            deviceID: this.deviceID,
            boxInfo: boxInfo
          }
          let addBags = axios.addBags(Client,obj);
          addBags.then(res => {
            if (res.data.State == "成功") {
              this.$message.success("添加成功");
              this.getAllData(this.deviceID);
          ...
          this.$message.warning("编号前缀和名称不能为空");
        }
      }else{
        this.$message.warning("编号起止输入错误")
      }
    }
  }
};
