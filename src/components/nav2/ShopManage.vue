        ...
        <dl class="text">
          ...
          <dd class="item">
            <el-form-item prop="C_Price" class="C_Price">
              <el-input v-model="ruleForm.C_Price" clearable></el-input>
            </el-form-item>
          </dd>
        </dl>
        <dl class="text">
...
export default {
  ...
  methods: {
    ...
    getAllData() {
      ...
      let getShopManage = axios.getShopManage(Client);
      let zhanwei = './static/img/zhanwei.jpg';
      getShopManage.then((res) => {
    ...
    getType(){
      ...
      if(this.TypeOptions != null && this.TypeOptions.length != 0) {
        let Client = localStorage.getItem("ClientAddr");
        let ShopManageType = axios.ShopManageType(Client);
        ShopManageType.then((res) => {
          if(res.data.State == '成功'){
    ...
    addData() {
      let Client = localStorage.getItem("ClientAddr");
      let addShopManage = axios.addShopManage(Client);
      addShopManage.then((res) => {
        if(res.data.State == '成功'){
          this.getAllData();
          this.addOpen1();
        }else {
  ...
  //   },
    ...
    upData(formName) {
      this.$refs[formName].validate((valid) =>{
        if(valid) {
          ...
          let changeShopManage = axios.changeShopManage(Client,name,imgUrl,price,type,no);
          changeShopManage.then((res) => {
            if(res.data.State == '成功'){
              this.getAllData();
              this.changeOpen1();
    ...
    del(){
      let Client = localStorage.getItem("ClientAddr");
      let delShopManage = axios.delShopManage(Client, this.delNo);
      delShopManage.then((res) => {
        if(res.data.State == '成功'){
          this.delopen();
          this.getAllData();
        }else {
