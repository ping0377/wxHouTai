...
export default {
  ...
  methods: {
    ...
    getAllData() {
      ...
      let ShopList = axios.ShopList(Client);
      ShopList.then((res) => {
    ...
    addData() {
      let Client = localStorage.getItem("ClientAddr");
      let addShopList = axios.addShopList(Client);
    ...
    upData() {
      ...
      let changeShopList = axios.changeShopList(Client,name,no);
      changeShopList.then((res) => {
        if(res.data.State == '成功'){
          this.getAllData();
          this.changeOpen1();
        }else {
    ...
    del(){
      let Client = localStorage.getItem("ClientAddr");
      let delShopList = axios.delShopList(Client, this.delNo);
      delShopList.then((res) => {
        if(res.data.State == '成功'){
          this.delopen();
          this.getAllData();
        }else {
