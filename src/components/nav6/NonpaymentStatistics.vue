      ...
      <el-row :gutter="10" style="padding: 5px;">
        <el-col :span="12">
          <div class="grid-content bg-purple">
            <dl class="Text">
              <dd class="Item1">{{No_count}}单</dd>
              <dd class="Item2">未付款单数</dd>
            </dl>
          </div>
        </el-col>
        <el-col :span="12">
          <div class="grid-content bg-purple">
            <dl class="Text">
              <dd class="Item1">{{No_pay.toFixed(2)}}元</dd>
              <dd class="Item2">未付款单总价</dd>
            </dl>
          </div>
        </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    search(sTime,eTime,wNo,sNo) {
      ...
      let ShopNo = sNo;
      let data = [];
      let searchDataNPS = axios.searchDataNPS(Client,startTime,endTime,WorkerNo,ShopNo);
      searchDataNPS.then((res) => {
        if(res.data.State == '成功'){
    ...
    list() {
      if(this.listData != []) {
        this.No_count = this.listData.TotalCount; // 未付款单数
        this.No_pay = this.listData.TotalPrice; // 未付款单总价
      }
    },
    commData() {
    ...
    getRowData(row){
      this.CreateDt = row.CreateDt; // 下单时间
      this.No = row.No; // 交易单号
