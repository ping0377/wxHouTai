      ...
      <el-row :gutter="10" style="padding: 5px;">
        <el-col :span="8">
          <div class="grid-content bg-purple">
            <dl class="Text">
              <dd class="Item1">{{cont}}张</dd>
              <dd class="Item2">发卡数</dd>
            </dl>
          </div>
        </el-col>
        <el-col :span="8">
          <div class="grid-content bg-purple">
            <dl class="Text">
              <dd class="Item1">{{gbM.toFixed(2)}}元</dd>
              <dd class="Item2">工本费</dd>
            </dl>
          </div>
        </el-col>
        <el-col :span="8">
          <div class="grid-content bg-purple">
            <dl class="Text">
              <dd class="Item1">{{cash.toFixed(2)}}元</dd>
              <dd class="Item2">现金支付</dd>
            </dl>
          </div>
        </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    search(sTime,eTime,wNo,sNo) {
      ...
      let ShopNo = sNo;
      let data = [];
      let searchDataCGS = axios.searchDataCGS(Client,startTime,endTime,WorkerNo,ShopNo);
      searchDataCGS.then((res) => {
        if(res.data.State == '成功'){
          ...
          for(var i=0;i<data.length;i++){
            ...
            data[i].gbMoney = data[i].gbMoney.toFixed(2);
          }
          this.allData = data;
          this.cont = this.allData.length;
          this.gbM = 0.0; //数值初始化
          this.cash = 0.0; //数值初始化
          for(let i=0;i<this.allData.length;i++){
            this.gbM += parseFloat(this.allData[i].gbMoney);
            this.cash +=parseFloat(this.allData[i].gbMoney);
          }
          this.commData();
        }else{
          this.$message.error(res.data.Reason);
